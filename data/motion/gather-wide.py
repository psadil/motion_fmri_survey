...
def add_filtered(
    ...
    for _, group in d_long.group_by(*cols, "variable"):
        peaks_to_use = peaks_out.join(group, on=col_for_peak, how="semi").to_dicts()[0]
        ...
        filtered.append(
            pl.DataFrame(
                group.select(*cols, "variable", "t").with_columns(value=_filtered)
            )
        )
...
def add_fd(d: pl.DataFrame, cols: list[str]) -> pl.DataFrame:
    ...
    d_fd = _add_fd(d, filtered=False, cols=cols).join(d_filtered_fd, on=[*cols, "t"])
